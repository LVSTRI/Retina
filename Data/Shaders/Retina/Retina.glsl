...
#define RetinaDeclarePushConstant() layout (scalar, push_constant) readonly uniform SPushConstant

#define RetinaNonUniform(expr) nonuniformEXT(expr)

bool RetinaIsHandleValid(in uint handle) {
  return handle != uint(-1);
}
